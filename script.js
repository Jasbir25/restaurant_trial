...
let orders = JSON.parse(localStorage.getItem('orders')) || []; // Load existing orders from local storage
let currentFilter = 'all';
let tableNumber; // Variable to hold the table number

function initializeApp() {
    tableNumber = getTableNumberFromURL(); // Fetch the table number from the URL
    createCategoryList();
    loadAllContent();
...
function getTableNumberFromURL() {
    ...
    return urlParams.get('tableNumber'); // Fetch the table number from the URL
}
...
function placeOrder() {
    ...
    const order = { 
        ...
        sgst: sgst,
        total: total,
        tableNumber: tableNumber // Add table number to the order
    };
...
function updateOrders() {
    ...
    orders.forEach((order, index) => {
        ...
        orderElement.innerHTML = `
            <h3>Order ${index + 1} (Table ${order.tableNumber})</h3> <!-- Display table number -->
            <ul>
